...
export const venues = mysqlTable("venues", {
  ...
  // Composite index for GPS-based venue lookups (latitude + longitude)
  gpsIdx: index("idx_venues_gps").on(table.latitude, table.longitude),
}));
...
export const concerts = mysqlTable("concerts", {
  ...
  // Composite index for dashboard query performance (userId + date DESC)
  userDateIdx: index("idx_concerts_user_date").on(table.userId, table.concertDate),
...
export const photos = mysqlTable("photos", {
  ...
  // Composite index for concert detail page queries (concertId + starred + createdAt DESC)
  concertStarredCreatedIdx: index("idx_photos_concert_starred_created").on(table.concertId, table.isStarred, table.createdAt),
}));
...
export const unmatchedPhotos = mysqlTable("unmatched_photos", {
  ...
  // Composite index for photo review page queries (userId + createdAt DESC)
  userCreatedIdx: index("idx_unmatched_photos_user_created").on(table.userId, table.createdAt),
}));
...
export const scanCache = mysqlTable("scan_cache", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("userId").notNull().unique(),
  totalDriveFiles: int("totalDriveFiles").notNull().default(0),
  lastUpdated: timestamp("lastUpdated").defaultNow().notNull(),
}, (table) => ({
  userIdx: index("scan_cache_user_idx").on(table.userId),
}));

export type ScanCache = typeof scanCache.$inferSelect;
export type InsertScanCache = typeof scanCache.$inferInsert;
