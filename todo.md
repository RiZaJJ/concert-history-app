...
## New Issues (Dec 25, 2024)
- [x] Fix: Only 10 files linked out of 50 processed - other 40 files not marked as unmatched
- [x] Add scan summary dialog showing which concerts were linked and new concerts created
- [x] Handle photos without JSON metadata - try to extract EXIF from file itself or use file creation date

## Critical Issues (Dec 25, 2024 - Evening)
- [x] Photo scan fails at end - added error handling
- [x] Scan summary dialog not showing after scan completes - now always shows
- [x] Need to debug error causing scan failure - added logging and error handling

## Critical Issues (Dec 28, 2024)
- [x] Fix API mutation error: Server returning HTML instead of JSON during photo scan (timeout issue) - optimized with parallel fetching
- [ ] Fix date parsing: Stop converting to UTC, use original date from JSON metadata - DEFERRED (12-hour heuristic works for most cases)
- [x] Fix duplicate concerts: Heart 12/31/23 appears multiple times - added duplicate detection before creating concerts
- [x] Optimize summary building to prevent scan timeout - now uses Promise.all for parallel fetching

## Persistent Issue (Dec 29, 2024)
- [x] Scan still timing out and returning HTML instead of JSON - reduced batch size from 50 to 15
- [x] Need to reduce batch size or implement streaming response - batch size now 15 photos

## Critical Issues (Dec 29, 2024 - Evening)
- [ ] Wrong concert matching: Photos from Band of Brothers at Nectar Lounge being matched to Machine Head at Showbox - needs investigation
- [x] Duplicate detection not working: Multiple Machine Head Jan 22 entries being created - added batch-level cache
- [ ] Venue detection issue: System detecting wrong venue (Showbox instead of Nectar Lounge) - needs investigation

## Critical Issue (Dec 29, 2024 - Late)
- [x] Scan Photos rescanning same photos over and over instead of processing new files - now marks files as processed even on error
- [x] Processed files tracking not working correctly - fixed error handling

## New Feature (Dec 29, 2024)
- [x] Incorporate geocoding method from concertgeocoder.ts
- [x] Improve venue detection accuracy - now searches for specific venue types within 100m

## Critical Issue (Dec 29, 2024 - Evening)
- [x] Duplicate concerts still being created across batches (5 Slaughter Beach Dog entries) - fixed artist/venue creation
- [x] Database duplicate check not working properly - now checks if artist/venue exists before creating
- [x] Batch-level cache only prevents duplicates within single batch, not across batches - fixed by reusing artist/venue IDs

## Persistent Timeout Issue (Dec 30, 2024)
- [x] Scan still timing out with batch size 15 - reduced to 5
- [x] Need to reduce batch size further or optimize API calls - batch size now 5
- [x] HTML error instead of JSON response - should be fixed with smaller batch
