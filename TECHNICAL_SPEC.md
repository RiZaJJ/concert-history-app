...
Caches total Google Drive file count per user to eliminate slow API pagination on every dashboard load.

| Column | Type | Constraints | Description |
|--------|------|-------------|-------------|
| id | INTEGER | PRIMARY KEY, AUTO_INCREMENT | Unique cache identifier |
| userId | INTEGER | FOREIGN KEY → users(id), UNIQUE, NOT NULL | Cache owner |
| totalDriveFiles | INTEGER | DEFAULT 0, NOT NULL | Total files in user's Drive folder |
| lastUpdated | TIMESTAMP | DEFAULT NOW(), NOT NULL | When cache was last refreshed |

**Indexes:** `userId` (unique)

**Purpose:** Dashboard displays "X of Y files scanned" stat. Without cache, this requires querying Google Drive (5-10 seconds for 18,000+ files). With cache, it's instant (50ms database query). Cache updated automatically during each scan.
...
- Files are listed in pages of **1000** (Drive API maximum), using `pageToken` pagination
- Total file count cached to `scan_cache` table after first scan (eliminates 5-10s load time)
...
- Pagination uses `nextPageToken` to handle folders with 18,000+ files
- User selects batch size (1-100 photos) per scan for controlled processing
...
- User-controlled batch processing (1-100 files per scan)
- File count caching (no repeated pagination for stats)

### OpenStreetMap (OSM) Overpass API

**Purpose:** **Primary venue detection** using open-source geographic data (replaced Google Maps for venue detection).
...
The system uses OSM Overpass API to find venues within 600m radius of GPS coordinates. Query targets multiple OSM tags for comprehensive venue coverage:

**Venue Tags Searched:**
- `amenity=music_venue` - Music-specific venues
- `amenity=theatre`, `amenity=nightclub`, `amenity=events_venue` - Performance spaces
- `leisure=stadium`, `leisure=sports_centre` - Large venues
- `tourism=attraction` - Tourist destinations (often major venues)
- `building=stadium`, `building=sports_hall` - Stadium buildings

**Distance Filtering:**
- Search radius: **600 meters** (0.373 miles)
- Results sorted by distance (closest first)
- Haversine formula used for precise distance calculation

**Venue Detection Flow:**

1. **Check database cache** (`venues` table with GPS coordinates)
   - Query: `SELECT * FROM venues WHERE distance(lat, lng) <= 600m`
   - If found → return instantly (0 API calls)

2. **Query OSM** if not cached
   - Send Overpass query with GPS center point and 600m radius
   - Parse XML/JSON response for venue names and coordinates

3. **Cache to database**
   - Store venue name, GPS, city, state, country
   - Future scans at same location = instant (cached)

**Rate Limiting:** OSM Overpass API is generous but monitored:
- No API key required
- Database caching eliminates 95%+ of queries
- Batch processing: ONE query per photo group (not per photo)

### OpenWeather API

**Purpose:** Reverse geocoding (GPS → city/state/country) - simpler and faster than Google Maps.

**Authentication:** API key in `OPENWEATHER_API_KEY` environment variable

**Key Operations:**

| Operation | Endpoint | Parameters | Response |
|-----------|----------|------------|----------|
| Reverse Geocode | `/geo/1.0/reverse` | `lat`, `lon`, `limit=1` | City, state, country names |

**Advantages over Google Maps:**
- Faster response times (simple JSON structure)
- Free tier sufficient for hobby use
- No complex billing or quotas
- City/state/country extraction in single call
...
The photo scanning process is the heart of the application, automatically ingesting photos from Google Drive and attempting to match them to concerts using intelligent grouping and caching.

**Step-by-Step Process:**

1. **Fetch Files from Google Drive**
   - Query specified folder for ALL files in pages of 1000 (handles 18,000+ file libraries)
   - Cache total file count to `scan_cache` table for instant dashboard stats
   - Filter for image files (JPEG, PNG, HEIC, WebP, DNG) and JSON sidecar metadata
   - Exclude already-processed files using `processed_files` table
   - Apply user-selected batch limit (1-100 photos)
...
2. **Extract EXIF Metadata**
   ...
   - Store all photos with EXIF in memory for grouping

3. **Group Photos by Date + GPS Location**
   - Generate grouping keys: `YYYY-MM-DD|lat,lng` (lat/lng rounded to 3 decimals = ~100m precision)
   - Apply midnight adjustment: photos taken 00:00-04:00 grouped with previous day
   - Result: 50 photos from Madison Square Garden on same night = 1 group
   - Benefit: ONE API call per group instead of 50 individual calls

4. **Reverse Geocode Location (One per Group)**
   - Use OpenWeather reverse geocoding API (faster than Google Maps)
   - Extract `city`, `state`, `country` components
   - Cached per group - all photos in group share same location data

5. **Detect Venue (One per Group)**
   - **Check database cache first** using `findVenuesNearCoordinates(lat, lng, 600m)`
   - If cached venue found → instant! (no API call)
   - If not cached → Query OpenStreetMap (OSM) Overpass API for venues within 600m
   - OSM query targets: `amenity=music_venue`, `leisure=stadium`, `tourism=attraction`, etc.
   - Store detected venue name in group for all photos
   - **Cache venue to database** with GPS coordinates for future instant lookups
   - Store `venueName`, `venueConfidence='high'`, `venueDetectionMethod='osm_scan'`

6. **Search for Matching Concert (One per Group)**
   - First check existing user concerts in memory (pre-loaded at scan start)
   - If no match, query setlist.fm API with:
     - **Venue name** (from OSM detection)
     - **Date** (adjusted for midnight if needed)
     - **GPS coordinates** (for distance filtering)
   - setlist.fm returns concerts at that venue on that date
   - Apply GPS distance filter: 1200m for searches without venue, skipped if venue provided
   - If concert found, create/update in database and cache in memory

7. **Proximity-Based Auto-Linking**
   - If still no concert match, check `photos` table for nearby photos on same date
   - Query: "Find photos within 500m on same date that are already linked"
   - If found → auto-link to that concert (handles 50 photos from same show)
   - Logged as "Auto-linked (nearby photo)"

8. **Link Photos or Create Unmatched**
   - **If concert found**: Create `photos` records, update stats
   - **If no concert**: Create `unmatched_photos` with detected venue for review
   - Mark files as processed in `processed_files` table

9. **Report Real-Time Progress**
   - Update in-memory progress tracker (`scanProgress` module) with:
     - Current photo number and filename
     - Location (city, state, country)
     - Detected venue name (e.g., "Madison Square Garden")
     - Status ("Matching concert...", "Linked to concert", "Auto-linked", "Unmatched")
     - Running stats (linked, new concerts, unmatched)
   - Frontend polls progress every 500ms via `getScanProgress` tRPC endpoint
   - Progress displayed in blue card with animated spinner and detailed status

**Performance Optimizations:**

- **Intelligent grouping**: 50 photos → 1-5 API calls instead of 150 (venue + geocode + setlist per photo)
- **Database venue cache**: Second scan of same venue = 0 OSM calls (instant!)
- **File count cache**: Dashboard loads in 50ms instead of 5-10 seconds
- **Batch processing**: User selects 1-100 photos per scan to control load
- **Processed file tracking**: Eliminates redundant API calls
- **Pre-loaded concert/venue data**: In-memory matching before expensive API calls
- **Proximity auto-linking**: 500m radius prevents duplicate concert creation
